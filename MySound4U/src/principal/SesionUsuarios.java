...
public class SesionUsuarios extends Sesion {
	...
	public void subirCancion(String nombre) {
		Cancion c;
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				Files.copy(origen.toPath(), new FileOutputStream(Cancion.PATH + origen.getName()));

				c = new Cancion(nombre, origen.getName(), (UsuarioRegistrado) usuario);
				api.addCancion(c);
				((UsuarioRegistrado) usuario).addCancion(c);
	...
	public void reproducir(Cancion cancion) {
		int edad = ((UsuarioRegistrado) usuario).getEdad();
		if (cancion.esBloqueda() || (!cancion.esValidada() && cancion.getAutor() != usuario)
				|| (cancion.esExplicita() && edad < 18) || (!((UsuarioRegistrado) usuario).esPremium())
